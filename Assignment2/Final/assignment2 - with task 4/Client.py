...
class Client:
	...
	def createWidgets(self):
		...
		self.terminate = Button(self.master, width=15, padx=3, pady=3)
	...
	def describeMovie(self):
		...
		if self.state == self.INIT:
			if self.pauseEvent.isSet():
				tkMessageBox.showerror("Describe", "Movie is not been playing, please play movie first.")
		
		elif self.state == self.READY:
			if self.pauseEvent.isSet():
				# send rtsp/tcp request to server
				self.sendDescribeRtsp()
	...
	def sendDescribeRtsp(self):
		"""Send DESCRIBE request over RTSP/TCP """
		self.CSeq += 1
		# create dict
		dictData = {}
		dictData['action'] = "DESCRIBE"
	...
	def playMovie(self):
		"""PLAY button handler."""
		# Task 4 question 2: setup movie when press play button
		if self.state == self.INIT:
			if self.pauseEvent.isSet():
				# send rtsp/tcp request to server
				self.sendSetupRtsp()
				if tkMessageBox.askokcancel("Play", "Click OK to continue."):
					...
					# listen for RTP/UDP socket thread
	...
	def sendPlayRtsp(self):
		...
		# start palying time in seconds
		self.playingTime = time.time()
		# print "playingTime: " + str(self.playingTime)
		self.CSeq += 1
		# create dict
		dictData = {}
	...
	def pauseMovie(self):
		...
		if self.state == self.INIT:
			if self.pauseEvent.isSet():
				tkMessageBox.showerror("Pause", "Movie is not been playing, please play movie first.")
		elif self.state == self.READY:
			if self.pauseEvent.isSet():
	...
	def processRtspReply(self,receivedRtspDataJSON):
		...
		# handle the received description data of stream
		elif receivedRtspData.has_key('Describe'):
			self.state = self.READY
			self.pauseEvent.set()
			self.stream = receivedRtspData['stream']
			self.encoding = receivedRtspData['encoding']
			tkMessageBox.showerror("Describe", "Stream is: %s, Encoding is: %s." %(self.stream, self.encoding) )
	...
	def rtpConnect(self):
		...
		try:
			...
			tkMessageBox.showwarning('Unable to Bind', 'Unable to bind PORT: %d, please restart the client.' % self.rtpPort)
			exit()
	...
	def listenRtp(self):
		...
		while True:
			try:
				...
				receivedRtpData= self.rtpSocket.recv(10240)
	...
	def updateScreen(self, frameNbr, payload):
		...
		# print "timeConsume: " + str(self.timeConsume) + " s"
		...
		self.label.configure(image = photo, height=322)
